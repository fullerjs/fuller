...
var Fuller = function(plan, opt) {
	...
	this.tools.files = FileTools;
	this.loadTools(plan.tools);
};

Fuller.prototype.requireTool = function(tool) {
	if(!toolsCache[tool]) {
		try {
			toolsCache[tool] = require('./fuller-' + tool);
		} catch(e) {
			console.log(e);
		}
	}
...
Fuller.prototype.cacheTools = function(tools) {
	var cache = {};

	for(var t in tools) {
		if(typeof tools[t] === 'object') {
			cache[t] = this.requireTool(t);
		} else {
			cache[tools[t]] = this.requireTool(tools[t]);
		}
	}
...
Fuller.prototype.loadTools = function(tools) {
	for(var t in tools) {
		if(!this.tools[t]) {
			var Tool = this.requireTool(t);
			this.tools[t] = new Tool(this, tools[t]);
		}
	}
};
