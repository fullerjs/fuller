...
Belt.prototype.parseConfig = function(tools, options, plan, root) {
  if (plan.tools) {
    let nextTools = plan.tools;

    for (let n in nextTools) {
      tools.push( this.getTool(nextTools[n], options) );
    }
  } else {
    let nextTools = getNextTools(plan);
    let next;
    ...
    for (let n in nextTools) {
      ...
      let nextTool = nextTools[n];
      let nextPlan = plan[nextTool];
      let opts = nextPlan.options ? extendOptions(nextPlan.options, options) : options;
